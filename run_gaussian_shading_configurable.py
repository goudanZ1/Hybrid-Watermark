...
from utils.image_utils import image_distortion, set_random_seed, transform_img
from utils.optim_utils import get_dataset, save_metrics
from utils.rid_utils import measure_similarity
from watermark_hybrid_configurable import ConfigurableHybridWatermarker
...
def main(args):
    ...
    watermark = ConfigurableHybridWatermarker(
        device=device,
        gs_ch_factor=args.channel_copy,
        gs_hw_factor=args.hw_copy,
        gs_channels=_parse_channels(args.gs_channels),
        ring_channels=_parse_channels(args.ring_channels),
        ring_radius=args.ring_radius,
        ring_radius_cutoff=args.ring_radius_cutoff,
        ring_value_range=args.ring_value_range,
        ring_threshold_dist=args.ring_threshold_dist,
        fpr_target=args.fpr,
    ...
    for i in tqdm(range(args.num)):
        ...
        if args.reference_model is not None:
            score = measure_similarity(
                [image_w], current_prompt, ref_model, ref_clip_processor, device
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Gaussian Shading (Configurable Hybrid)")
    ...
    parser.add_argument("--gs_channels", default=None, help="Comma-separated, e.g. 0,2,3")
    parser.add_argument("--ring_channels", default=None, help="Comma-separated, e.g. 1")
    parser.add_argument("--ring_radius", default=14, type=int)
    parser.add_argument("--ring_radius_cutoff", default=3, type=int)
    parser.add_argument("--ring_value_range", default=32, type=int)
    parser.add_argument("--ring_threshold_dist", default=None, type=float)
